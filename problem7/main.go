...
func (tc *testCase) solvePartOne() string {
	lines := parseLines(tc.input)
	lines = ensureParents(lines)
...
func (tc *testCase) solvePartTwo() string {
	...
	leaf := &treeLeaf{}
	buildTree(rootName, leaf, lines)
	leaf.weight = 0
	walkTree(leaf)

	//fmt.Printf("%# v", pretty.Formatter(leaf))

	//leaf2 := &treeLeaf{}
	//for _, elem := range leaf.children {
	//	if elem.name == "rilyl" {
	//		leaf2 = elem
	//		break
	//	}
	...
	walkTree2(leaf)
	fmt.Println(walkTree2Found)
	for _, xx := range walkTree2Found.children {
		fmt.Println("\t", xx)
	}
	// !!!!!!!!!!!!!!!!!!!!!!!!
	// 1993 is answer

	fmt.Println(walkTree2FoundParent)
	for _, xx := range walkTree2FoundParent.children {
		fmt.Println("\t", xx)
	}
...
var walkTree2Found *treeLeaf = nil
var walkTree2FoundParent *treeLeaf = nil

func walkTree2(leaf *treeLeaf) {
	walkTree2FoundParent = leaf
	if walkTree2Found != nil {
		return
	}

	for _, childLeaf := range leaf.children {

		min, max := 99999999, -1
		for _, elem := range leaf.children {
			if min > elem.weightAccum {
				min = elem.weightAccum
			}
			if max < elem.weightAccum {
...
func walkTree(leaf *treeLeaf) {
	for _, childLeaf := range leaf.children {
		walkTree(childLeaf)
		leaf.weightAccum += childLeaf.weightAccum
	}
	leaf.weightAccum += leaf.weight
}

func buildTree(leafName string, leaf *treeLeaf, elems []lineItem) {
	for _, elem := range elems {
		if leafName == elem.name {
			leaf.name = elem.name
			leaf.weight = elem.weight

			leaf.children = []*treeLeaf{}
			for _, child := range elem.children {
				childLeaf := &treeLeaf{}
				buildTree(child, childLeaf, elems)
...
func parseLines(lines string) []lineItem {
	result := []lineItem{}
	for _, line := range strings.Split(lines, "\n") {
		tmp0 := strings.Split(line, " -> ")
		left, right := "", ""
		children := []string{}
		if len(tmp0) == 2 {
			left, right = tmp0[0], tmp0[1]
			children = strings.Split(right, ", ")
		} else {
			left, right = tmp0[0], ""
		}

		tmp1 := strings.Split(left, " ")
		weight, _ := strconv.Atoi(strings.Trim(tmp1[1], "()"))
...
func ensureParents(elems []lineItem) []lineItem {
	for elemK, elem := range elems {
		for _, otherElem := range elems {
			if elem.name == otherElem.name {
				continue
			}

			found := false
			for _, child := range otherElem.children {
				if child == elem.name {
					found = true
					break
				}
			...
			if found {
				elems[elemK].parent = otherElem.name
			}
		}
	}
	return elems
