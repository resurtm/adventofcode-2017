...
var testCasesPartTwo = []testCase{
	{
		input: []string{"abcde fghij", "abcde xyz ecdab", "a ab abc abd abf abj",
...
func (tc *testCase) solvePartOne() int {
	valid := 0
	for _, words := range tc.input {
		if solvePartOneInternal(strings.Split(words, " ")) {
			valid += 1
		}
	}
	return valid
...
func solvePartOneInternal(words []string) bool {
	elems := []string{}
...
func (tc *testCase) solvePartTwo() int {
	valid := 0
	for _, words := range tc.input {
		if solvePartTwoInternal(strings.Split(words, " ")) {
			valid += 1
		}
	}
	return valid
...
func solvePartTwoInternal(words []string) bool {
	elems := [][]byte{}
	for _, word := range words {
		wordB := populateBytes(word)
		for _, elem := range elems {
			if len(wordB) == len(elem) && bytesCheck(wordB, elem) {
				return false
			}
		}
		elems = append(elems, populateBytes(word))
	}
	return true
}

func bytesCheck(a, b []byte) bool {
	sa, sb := map[byte]int{}, map[byte]int{}
	for _, x := range a {
		_, ok := sa[x]
		if !ok {
			sa[x] = 0
...
func populateBytes(s string) []byte {
	res := []byte{}
	for _, ch := range s {
